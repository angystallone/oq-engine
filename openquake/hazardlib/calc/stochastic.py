...
from openquake.baselib.general import AccumDict, sample_n_occ
...
# exercised in event_based/case_21
def sample_cluster(sources, srcfilter, num_ses, param):
    ...
    tot_num_occ = numpy.random.poisson(rate * time_span * samples *
                                       num_ses) # *src.smweight
    ...
    if sources.cluster:
        trt_smr, = sources[0].trt_smrs  # simplifying assertion
        rups = []
        for src in sources:
            assert src.trt_smr == trt_smr
            t0 = time.time()
            for rup in src.iter_ruptures():
                rup.source_id = src.source_id
                rup.seed = src.serial(ses_seed)
                rups.append(rup)
            dt = time.time() - t0
        ...
        eb_ruptures = [
            EBRupture(rup, rup.source_id, trt_smr, n_occ)
            for rup, n_occ in sample_n_occ(rups, tot_num_occ, ses_seed)]
